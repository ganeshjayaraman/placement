...
if (isset($_POST['submit'])) {
    ...
    if (!empty($user_username) && !empty($user_password) && !empty($user_cpassword) && !empty($user_email)) {
        ...
        if(strcmp($user_password,$user_cpassword)!=0){
            ...
            header("Refresh:2; url=register.php", true, 303);
        }
        $query="SELECT * FROM login where emailid='$user_email'";
        $result=mysqli_query($sql,$query)
        or die();
        if(!($row=mysqli_fetch_array($result))){
            $user_password = sha1($user_password).md5($user_email);
            $user_password = sha1($user_password);
            $query="INSERT INTO login(emailid,password,username,created_at)values('$user_email','$user_password','$user_username',NOW())";
            $result=mysqli_query($sql,$query);
            mysqli_close($sql);
            echo 'Registered!!!';
